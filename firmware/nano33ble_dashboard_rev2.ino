...
void setup() {
  Serial.begin(115200);
  // 주의: while(!Serial) 무한 대기는 배터리/단독 부팅 시 BLE 광고를 막는다.
  // 시리얼이 열리면 부팅 배너를 보여주되, 최대 3초만 기다리고 진행.
  for (unsigned long t0 = millis(); !Serial && millis() - t0 < 3000; ) {}
  ...
  if (!BLE.begin()) {
    ...
    BLE.setDeviceName(deviceName.c_str());   // GAP 기기 이름 — macOS 등에서 "Arduino" 대신 BLE-XXXX 표시
    BLE.setLocalName(deviceName.c_str());    // 스캔 응답에 실리는 이름 (macOS 매칭 경로)
    // 서비스 광고는 유지해야 한다: ArduinoBLE는 이름을 광고 패킷으로 옮겨주지 않으므로
    // (이름은 항상 스캔 응답), 광고 패킷의 서비스 UUID가 Windows Chrome의 유일한 매칭 수단.
