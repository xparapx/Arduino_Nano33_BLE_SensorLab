# -*- coding: utf-8 -*-
"""Nano 33 BLE Sense Rev2 — phyphox 실험 파일 세트 생성기"""
...
# 디자인 시스템 (phyphox 다크 테마 기준 고채도 팔레트)
...
def fhex(v):  # float32 little-endian hex
    return struct.pack("<f", v).hex()

def bt_input(choice, buffers):
    out = [f'        <config char="{CONF_CH}" conversion="hexadecimal">{fhex(choice)}</config>']
    for i, b in enumerate(buffers):
        out.append(f'        <output char="{DATA_CH}" conversion="float32LittleEndian" offset="{4*i}">{b}</output>')
    body = "\n".join(out)
    return f'''    <input>
...
def graph(label, ylab, yunit, curves, style="lines", lw="1.5"):
    """curves: list of (xbuf, ybuf, color|None)"""
    ins = []
...
def doc(fname, title, desc, conts, inp, views, exp):
    v = "\n".join(views)
...
# ---------- 벡터형 (가속도/자이로/자기장) ----------
def vector_exp(fname, choice, title, desc, unit, comp_note):
    conts = ["t", "vx", "vy", "vz", "vm"]
    views = [
        view("그래프", [
    ...
    return doc(fname, f"{title} (Nano BLE)", desc, conts, bt_input(choice, conts), views, exp)

vector_exp("01_acceleration.phyphox", 1.0, "가속도",
    "BMI270 IMU로 3축 가속도를 측정합니다. 정지 상태에서 |a| ≈ 1 g 입니다.",
    "g", "센서: BMI270, 중력 포함 가속도. 1 g = 9.81 m/s²")
vector_exp("02_gyroscope.phyphox", 2.0, "자이로스코프",
    "BMI270 IMU로 3축 각속도를 측정합니다.",
    "°/s", "센서: BMI270, 각속도(회전 속도)")
vector_exp("03_magnetometer.phyphox", 3.0, "자기장",
    "BMM150으로 3축 자기장을 측정합니다. 지구 자기장은 약 25~65 µT입니다.",
    "µT", "센서: BMM150. 주변 전자기기·철제 구조물의 영향에 주의")

# ---------- 환경 통합 (기압+온도 2종+습도) ----------
conts = ["t", "p", "t1", "t2", "rh"]
doc("04_environment.phyphox", "환경 (기압·온도·습도)",
    "LPS22HB(기압·온도)와 HS3003(온도·습도)을 동시에 측정합니다.",
    conts, bt_input(11.0, conts),
    [
        view("그래프", [
            graph("기압", "기압", "hPa", [("t","p",CP)], lw="2"),
            graph("온도 (센서 2종 비교)", "온도", "°C",
                  [("t","t1",CT1),("t","t2",CT2)]),
            graph("상대 습도", "습도", "%", [("t","rh",CH)], lw="2"),
        ]),
        view("측정값", [
            info("실시간 측정값", bold=True), SEP,
            value("기압", "p", "hPa", CP, precision="1"),
            value("온도 (기압계 LPS22HB)", "t1", "°C", CT1),
            value("온도 (습도계 HS3003)", "t2", "°C", CT2),
            value("상대 습도", "rh", "%", CH, precision="1"),
        ]),
        view("안내", GUIDE + [SEP,
            info("두 온도 센서의 값 차이는 보드 내 발열·센서 특성 차이입니다."),
            info("기압으로 고도 변화(약 -0.12 hPa/m)를 관찰할 수 있습니다.")]),
    ],
    export("환경 데이터", [("t (s)","t"),("기압 (hPa)","p"),
        ("온도_기압계 (°C)","t1"),("온도_습도계 (°C)","t2"),("습도 (%)","rh")]))

# ---------- 색상 (RGB) ----------
conts = ["t", "amb", "cr", "cg", "cb"]
doc("05_color.phyphox", "색상 (RGB)",
    "APDS9960으로 빛의 R·G·B 성분을 측정합니다. 성분별 차트와 수치를 제공합니다.",
    conts, bt_input(6.0, conts),
    [
        view("그래프", [
...
gmaps = [(None, 0.5, "대기 중"),
         ...
         (2.5, 3.5, "↑ 아래에서 위로"),
         (3.5, None, "↓ 위에서 아래로")]
doc("07_gesture.phyphox", "제스처",
    "APDS9960 위에서 손을 움직이면 방향(왼쪽/오른쪽/위/아래)을 인식합니다.",
    conts, bt_input(12.0, conts),
    [
        view("제스처", [
            info("센서 위 3~10 cm에서 손을 한 방향으로 움직이세요.", bold=True), SEP,
            value("인식된 동작", "gest", "", CM, size="3", precision="0", maps=gmaps),
            SEP,
            graph("제스처 이력 (1=왼쪽 2=오른쪽 3=아래→위 4=위→아래)",
                  "코드", "", [("t","gest",CM)], style="dots", lw="3"),
        ]),
        view("안내", GUIDE + [SEP,
            info("방향은 USB 커넥터를 아래로 두었을 때 기준입니다."),
            info("인식이 안 되면 손을 조금 더 천천히, 가까이에서 움직여 보세요.")]),
    ],
    export("제스처 데이터", [("t (s)","t"),("제스처 코드","gest")]))

# ---------- 근접 ----------
conts = ["t", "prox"]
pmaps = [(None, 30, "매우 가까움"), (30, 100, "가까움"),
         (100, 200, "중간"), (200, None, "멀음")]
doc("08_proximity.phyphox", "근접",
    "APDS9960으로 물체와의 상대 거리를 측정합니다. 0(가까움)~255(멀음).",
    conts, bt_input(13.0, conts),
    [
        view("그래프", [graph("근접값 (작을수록 가까움)", "근접", "", [("t","prox",CPROX)], lw="2")]),
        view("측정값", [
            value("근접값", "prox", "", CPROX, size="3", precision="0"),
            SEP,
...
doc("09_sound.phyphox", "음량",
    ...
    [
        ...
        view("측정값", [
            value("음량", "db", "dBFS", CSND, size="3", precision="1"),
        ]),
        view("안내", GUIDE + [SEP,
...
# ---------- 외장 아날로그 ----------
conts = ["t", "v0", "v1", "v2"]
doc("10_analog_external.phyphox", "외장 센서 (아날로그)",
    "A0·A1·A2 핀의 전압을 측정합니다. 외장 아날로그 센서를 연결해 사용하세요.",
    conts, bt_input(9.0, conts),
    [
        view("그래프", [
        ...
        view("안내", GUIDE + [SEP,
            ...
            info("물리량 환산(예: 전압→온도)은 펌웨어의 choice 9 부분에서 수식을 수정하거나,"),
            info("이 파일의 analysis 모듈로 앱에서 직접 계산할 수도 있습니다.")]),
    ],
